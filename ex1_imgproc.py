...
#img = cv2.imread('./PM5540_Philips_pattern.png', cv2.IMREAD_GRAYSCALE)
img = cv2.imread('./TE100.jpg', cv2.IMREAD_GRAYSCALE)   
img = img.astype(???) # Convert to floating point numbers
...
img_sub = img[ border_r:-border_r, border_c:-border_c ]
fig = plt.figure()
...
print yy.shape
fig = plt.figure()
plt.figimage(???) # Convert to integer and display it in grayscsale
plt.show()

#img_u8 = cv2.convertScaleAbs(xx.astype(int))
img_u8 = cv2.imread('./lena_gray_512.tif', cv2.IMREAD_GRAYSCALE)
...
for ax, k in zip(axs.flat, range(6)):
    scale_factor = 1./(2**k)
    img_small = cv2.resize(img_u8, ??? ) # Resize image
...
# Number in intensity levels, number of bits (bit depth). Artefacts
...
# Need to reorient them appropriately
...
# Convert from (-b,b) to (0,255) and save
??? = cv2.minMaxLoc(grad_x) # Use this command
...
# %% Colorize the magnitude and direction (optional)
# Show the tiger image...


# %% Temporal gradient
# Need two images for this (from the DAVIS)
...
# First attempt:
#img1 = cv2.imread('/home/ggb/Documents/ex1/slider_depth/images/frame_00000059.png', cv2.IMREAD_GRAYSCALE)
#img2 = cv2.imread('/home/ggb/Documents/ex1/slider_depth/images/frame_00000060.png', cv2.IMREAD_GRAYSCALE)

# More polished and flexible way of reading images:
id0 = 1
...
# Display two images and temporal derivative
fig = plt.figure()
